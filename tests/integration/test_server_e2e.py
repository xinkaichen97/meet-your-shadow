...
# The Orchestrator's first rule always requires 'answers' already in state
# (real usage seeds this via record_answers() before the first turn, see
# app/fast_api_app.py's /api/answers) — a session with no answers crashes
# AnalystAgent with a KeyError, so every test below seeds a full answer set.
_SAMPLE_STATE = {
...
def test_adk_run_sse(server_fixture: subprocess.Popen[str]) -> None:
    ...
    session_data = {"state": _SAMPLE_STATE}
    ...
    data = {
        ...
        "new_message": {"role": "user", "parts": [{"text": "Here are my answers."}]},
        "streaming": True,
    }
...
def test_a2a_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    # A2A's contextId maps to the underlying ADK session_id, and — with no
    # auth configured — the ADK a2a executor derives user_id as
    # f"A2A_USER_{context_id}" (see google.adk.a2a.converters
    # .request_converter._get_user_id). Pre-creating the session under that
    # exact user_id/session_id, seeded with answers (same as the /run_sse
    # test above), lets this exercise the real flow instead of crashing on
    # missing state.
    ...
    session_response = requests.post(
        f"{BASE_URL}/apps/app/users/{user_id}/sessions",
        headers=HEADERS,
        json={"state": _SAMPLE_STATE, "session_id": context_id},
        timeout=60,
    )
    assert session_response.status_code == 200
    ...
    message = Message(
        ...
        role=Role.user,
        parts=[Part(root=TextPart(text="Here are my answers."))],
    )
